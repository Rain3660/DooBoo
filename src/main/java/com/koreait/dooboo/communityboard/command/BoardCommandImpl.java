...
public class BoardCommandImpl implements BoardCommand{
	...
	public boolean deleteBoard(long ldx) {
		int queryResult = 0;

		CommunityBoardDTO board = communityBoardDAO.selectBoardDetail(ldx);
		
		if (board != null && "N".equals(board.getDelYn())) {
			queryResult = communityBoardDAO.deleteBoard(ldx);
		}
	...
	public Map<String,Object> getBoardList(CommunityBoardDTO boardDTO) {
		...
		if (boardTotalCount > 0) {
			...
			for(CommunityBoardDTO a : boardList) {
